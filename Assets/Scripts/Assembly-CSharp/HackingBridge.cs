...
public class HackingBridge : MonoBehaviour
{
	...
	public GameObject m_directionalArrow;
	...
	{
		bool flag = ((m_connectedNode1.m_connections.Contains(m_connectedNode2) && m_connectedNode2.m_connections.Contains(m_connectedNode1)) ? true : false);
		BridgeSpriteInfo bridgeSpriteInfo = ((!flag) ? m_hackingSystem.m_bridgeDottedLine : m_hackingSystem.m_bridgeLine);
		...
		if (flag)
		{
			if ((bool)m_directionalArrow)
			{
				Object.DestroyImmediate(m_directionalArrow);
			}
		}
		else if (!m_directionalArrow)
		{
			m_directionalArrow = Object.Instantiate(GameObject.Find("Global").GetComponent<Hacking_Globals>().m_BridgeDirectionalArrowPrefab) as GameObject;
			m_directionalArrow.transform.parent = base.gameObject.transform;
			m_directionalArrow.transform.localScale = Vector3.one;
			num = Vector3.Distance(m_connectedNode1.gameObject.transform.position, m_connectedNode2.gameObject.transform.position);
			Vector3 vector = m_connectedNode2.gameObject.transform.position - m_connectedNode1.gameObject.transform.position;
			vector = m_connectedNode1.gameObject.transform.position + vector.normalized * (num * 0.5f);
			m_directionalArrow.transform.localPosition = m_connectedNode2.gameObject.transform.position - vector;
			bool flag2 = m_connectedNode1.m_connections.Contains(m_connectedNode2);
			m_directionalArrow.transform.rotation = Quaternion.Euler(0f, 0f, ((!flag2) ? 270f : 90f) + angle);
		}
	}

	public void ReSetupDirectionalArrow()
	{
		if ((bool)m_directionalArrow)
		{
			GameObject gameObject = Object.Instantiate(GameObject.Find("Global").GetComponent<Hacking_Globals>().m_BridgeDirectionalArrowPrefab) as GameObject;
			gameObject.transform.parent = m_directionalArrow.transform.parent;
			gameObject.transform.localScale = m_directionalArrow.transform.localScale;
			gameObject.transform.localPosition = m_directionalArrow.transform.localPosition;
			gameObject.transform.rotation = m_directionalArrow.transform.rotation;
			Object.DestroyImmediate(m_directionalArrow);
			m_directionalArrow = gameObject;
		}
	}

	public void ReSetupBridge()
	{
	}
	...
	private void SetBridgeProgress(BridgeConnection connection, float progress)
	{
		...
		{
			BridgeSpriteInfo bridgeSpriteInfo = ((!m_connectedNode1.m_connections.Contains(m_connectedNode2) || !m_connectedNode2.m_connections.Contains(m_connectedNode1) || 1 == 0) ? m_hackingSystem.m_bridgeDottedLine : m_hackingSystem.m_bridgeLine);
			connection.m_bridgeProgressSprite.width = connection.m_bridgeSprite.width * progress;
			Vector2 pixelDimensions = new Vector2(connection.m_bridgeSprite.pixelDimensions.x * progress, connection.m_bridgeSprite.pixelDimensions.y);
			connection.m_bridgeProgressSprite.SetPixelDimensions(pixelDimensions);
			...
			connection.m_bridgeProgressSprite.UpdateUVs();
		}
	}

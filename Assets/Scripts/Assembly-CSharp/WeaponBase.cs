...
public abstract class WeaponBase : MonoBehaviour
{
	...
	[HideInInspector]
	public WeaponItemID m_WeaponItemID = WeaponItemID.None;
	...
	private int m_ReloadingAmmo;

	private int m_StartFireCount;

	private float m_HolsterTimer;
	...
	[HideInInspector]
	public float m_FirstPersonAnimationSpeed;

	[HideInInspector]
	public Vector3 m_CrouchModelLeftPosition = PlayerController.m_DefaultCrouchModelLeftPosition;

	[HideInInspector]
	public Vector3 m_CrouchModelLeftRotation = PlayerController.m_DefaultCrouchModelLeftRotation;

	[HideInInspector]
	public Vector3 m_CrouchModelRightPosition = PlayerController.m_DefaultCrouchModelRightPosition;

	[HideInInspector]
	public Vector3 m_CrouchModelRightRotation = PlayerController.m_DefaultCrouchModelRightRotation;

	[HideInInspector]
	public Vector3 m_StandModelLeftPosition = PlayerController.m_DefaultStandModelLeftPosition;

	[HideInInspector]
	public Vector3 m_StandModelLeftRotation = PlayerController.m_DefaultStandModelLeftRotation;

	[HideInInspector]
	public Vector3 m_StandModelRightPosition = PlayerController.m_DefaultStandModelRightPosition;

	[HideInInspector]
	public Vector3 m_StandModelRightRotation = PlayerController.m_DefaultStandModelRightRotation;

	[HideInInspector]
	public Vector3 m_UpOverModelLeftPosition = PlayerController.m_DefaultUpOverModelLeftPosition;

	[HideInInspector]
	public Vector3 m_UpOverModelLeftRotation = PlayerController.m_DefaultUpOverModelLeftRotation;

	[HideInInspector]
	public Vector3 m_UpOverModelRightPosition = PlayerController.m_DefaultUpOverModelRightPosition;

	[HideInInspector]
	public Vector3 m_UpOverModelRightRotation = PlayerController.m_DefaultUpOverModelRightRotation;
	...
	public Renderer m_RendererThirdPersonEnemy;

	public Material m_DeathMaterial;

	protected string m_FireSound3rdEventName;

	protected string m_FireSound3rdEventNameStop;

	protected string m_FireSound1stEventName;

	protected string m_FireSound1stEventNameStop;

	protected string m_ReloadSound3rdEventName;

	protected string m_ReloadSound1stEventName;

	protected string m_HolsterSoundEventName;

	protected string m_DrawSoundEventName;
	...
	protected GameObject m_MuzzleFlashAttachObject;
	...
	public void SetDeathMaterial()
	{
		if (m_RendererThirdPersonEnemy != null)
		{
			m_RendererThirdPersonEnemy.material = m_DeathMaterial;
		}
	}
	...
	public bool ThreatWithinPCR(float ThreatDistSqr)
	{
		if (m_WeaponItemID == WeaponItemID.None)
		{
			return false;
		}
		Item_Weapon item_Weapon = Globals.m_Inventory.m_Items[1][(int)m_WeaponItemID] as Item_Weapon;
		return ThreatDistSqr >= item_Weapon.m_MinEnemyPCRSqr && ThreatDistSqr <= item_Weapon.m_MaxEnemyPCRSqr;
	}

	public float HalfPCR()
	{
		Item_Weapon item_Weapon = Globals.m_Inventory.m_Items[1][(int)m_WeaponItemID] as Item_Weapon;
		return Mathf.Sqrt(0.5f * (item_Weapon.m_MaxEnemyPCRSqr - item_Weapon.m_MinEnemyPCRSqr));
	}

	public float AveragePCR()
	{
		Item_Weapon item_Weapon = Globals.m_Inventory.m_Items[1][(int)m_WeaponItemID] as Item_Weapon;
		return 0.5f * (item_Weapon.m_MaxEnemyPCRSqr + item_Weapon.m_MinEnemyPCRSqr);
	}

	public bool LowOnAmmo()
	{
		return m_LowAmmoThreshold > 0 && m_CurrentAmmo <= m_LowAmmoThreshold;
	}
	...
	protected virtual void Start()
	{
		if (m_User == Globals.m_PlayerController)
		{
			...
			Globals.m_HUD.SetCurrentAmmo(m_CurrentAmmo, Globals.m_PlayerController.GetAmmoForCurrentWeapon());
		}
	}
	...
	protected virtual void Update()
	{
		...
		case WeaponState.Aiming:
			...
			if (m_WeaponState == WeaponState.Firing && m_FireType != FireType.RapidFire && m_FireTimer > m_FireRate)
			{
				...
				m_FireTimer = 0f;
				ResetFOV(false);
				if (m_CurrentAmmo <= 0 && m_User != null)
				{
		...
		case WeaponState.Reloading:
			...
			{
				m_WeaponState = WeaponState.Idle;
				m_CurrentAmmo += m_ReloadingAmmo;
				if (m_User == Globals.m_PlayerController)
				{
					Globals.m_PlayerController.GiveAmmo(m_WeaponType, -m_ReloadingAmmo);
				}
				if (m_User != null)
				{
	...
	protected virtual void PlayerUpdate()
	{
		...
		case WeaponState.Holstering:
			m_HolsterTimer -= Time.deltaTime;
			if (m_HolsterTimer <= 0f)
			{
		...
		if (Globals.m_PlayerController.m_CameraMode == PlayerController.CameraMode.Third)
		{
			m_DesiredFOV = 60f;
		}
		...
		if (m_ZoomTimer >= m_ZoomTime && m_ZoomScale > 0f)
		{
			m_DesiredFOV = m_DefaultFOV / m_ZoomScale;
			m_DesiredWeaponFOV = m_DefaultWeaponFOV / m_ZoomScale;
		}
		if ((Globals.m_PlayerController.m_CoverState == PlayerController.CoverState.TransitioningToFire || Globals.m_PlayerController.m_CoverState == PlayerController.CoverState.CoverAiming || Globals.m_PlayerController.m_CoverState == PlayerController.CoverState.Firing || (Globals.m_PlayerController.m_CoverState == PlayerController.CoverState.Reloading && Globals.m_PlayerController.m_CameraCoverDot < Globals.m_PlayerController.m_CameraCoverAimLimit)) && m_PlayerCoverZoomScale > 0f)
		{
		...
		if (fieldOfView != m_DesiredFOV)
		{
			...
			Globals.m_CameraController.SetFOV(fieldOfView, fieldOfView2);
		}
	...
	public virtual void StartFire()
	{
		if (m_CurrentAmmo <= 0)
		{
			SoundManager.TriggerEvent("Play_Weapon_DryFire", base.gameObject);
		}
	...
	public virtual void EndFire()
	{
		...
		case FireType.RapidFire:
			...
			{
				SoundManager.TriggerEvent(m_FireSound3rdEventNameStop, base.gameObject);
			}
			else
			{
				SoundManager.TriggerEvent(m_FireSound1stEventNameStop, base.gameObject);
			}
			StopFireFX();
			m_FireTimer = 0f;
			m_WeaponState = WeaponState.Idle;
	...
	public virtual void PlayFireFX()
	{
		if (m_User != Globals.m_PlayerController || Globals.m_PlayerController.m_CameraMode == PlayerController.CameraMode.Third)
		{
			SoundManager.TriggerEvent(m_FireSound3rdEventName, base.gameObject);
		}
		else
		{
			SoundManager.TriggerEvent(m_FireSound1stEventName, base.gameObject);
		}
		if (m_MuzzleFlashFX != null)
		{
	...
	public virtual void PlayHitFX(Vector3 hitloc, Vector3 hitnorm, int hitlayer, bool isturret)
	{
		if ((hitlayer != 9 && hitlayer != 14) || isturret)
		{
			...
			SoundManager.TriggerEvent("Play_Weapon_Richochet", gameObject);
		}
		else if (hitlayer != 14)
		{
	...
	public virtual void ResetFOV(bool instant = false)
	{
		m_DesiredFOV = m_DefaultFOV;
		m_DesiredWeaponFOV = m_DefaultWeaponFOV;
		if (instant)
		{
			Globals.m_CameraController.SetFOV(m_DesiredFOV, m_DesiredWeaponFOV);
		}
	}
	...
	{
		return Mathf.Clamp((float)m_CurrentAmmo / (float)(Globals.m_Inventory.m_Items[1][(int)m_WeaponItemID] as Item_Weapon).m_AmmoPerClip, 0f, 1f);
	}

	public virtual void Reload(float reloadTime)
	{
		int num = (Globals.m_Inventory.m_Items[1][(int)m_WeaponItemID] as Item_Weapon).m_AmmoPerClip - m_CurrentAmmo;
		if (m_User == Globals.m_PlayerController && num > Globals.m_PlayerController.GetAmmoForCurrentWeapon())
		{
			num = Globals.m_PlayerController.GetAmmoForCurrentWeapon();
		}
		m_ReloadingAmmo = num;
		...
		if (m_User != Globals.m_PlayerController || Globals.m_PlayerController.m_CameraMode == PlayerController.CameraMode.Third)
		{
			SoundManager.TriggerEvent(m_ReloadSound3rdEventName, base.gameObject);
		}
		else
		{
			SoundManager.TriggerEvent(m_ReloadSound1stEventName, base.gameObject);
		}
	}
	...
	protected virtual void FireHitScanBullet()
	{
		...
		if (m_ProjectilePrefab == null)
		{
			if (flag)
			{
				...
				case 9:
				{
					CharacterBase characterBase2 = Globals.FindCharacterBase(hitInfo.collider.transform);
					if (characterBase2 != null)
					{
						bool headshot = false;
						RaycastHit hitInfo2;
						if (characterBase2.m_HeadCollider != null && characterBase2.m_HeadCollider.Raycast(ray, out hitInfo2, 100f))
						{
							headshot = true;
						}
						characterBase2.TakeDamage(new DamageData(m_User, base.transform.position, m_Damage, DamageType.Normal, headshot));
					}
					break;
				}
				case 14:
					Globals.m_PlayerController.TakeDamage(new DamageData(m_User, base.transform.position, m_Damage, DamageType.Normal, false));
					break;
				case 21:
					characterBase = Globals.FindCharacterBase(hitInfo.collider.transform);
					if (characterBase != null)
					{
						characterBase.TakeDamage(new DamageData(m_User, base.transform.position, m_Damage, DamageType.Normal, false));
					}
					break;
				case 16:
				{
					Mine component = hitInfo.collider.gameObject.GetComponent<Mine>();
					if (component != null)
					{
						component.Explode();
					}
			...
			RaycastHit hitInfo3;
			if ((!flag || hitInfo.collider.gameObject.layer != 14) && m_User != Globals.m_PlayerController && Globals.m_PlayerController.m_BulletWhizCollider.Raycast(ray, out hitInfo3, 100f))
			{
				GameObject gameObject = new GameObject("temp bullet whiz GO");
				gameObject.transform.position = hitInfo3.point;
				SoundManager.TriggerEvent("Play_Weapon_Whiz", gameObject);
		...
		else
		{
			...
			gameObject2.GetComponent<ProjectileBase>().m_Firer = this;
			gameObject2.GetComponent<ProjectileBase>().m_Owner = m_User;
		}
	}
	...
	public virtual void ToggleHostered()
	{
		if (m_WeaponState != WeaponState.Holstered)
		{
			...
			if (Globals.m_PlayerController.m_CameraMode == PlayerController.CameraMode.First)
			{
				SetFirstPersonAnim(FirstPersonAnimation.Holster, false, false);
				SoundManager.TriggerEvent(m_HolsterSoundEventName, base.gameObject);
			}
			else
			{
		...
		else if (m_WeaponState == WeaponState.Holstering || m_WeaponState == WeaponState.Holstered)
		{
			...
			if (Globals.m_PlayerController.m_CameraMode == PlayerController.CameraMode.First)
			{
				SetFirstPersonAnim(FirstPersonAnimation.Draw, false, false);
				SoundManager.TriggerEvent(m_DrawSoundEventName, base.gameObject);
			}
			else
			{
	...
	public virtual void SetFirstPerson()
	{
		...
		{
			SetFirstPersonAnim(FirstPersonAnimation.Idle, true, false);
		}
	}
	...
	public virtual void SetFirstPersonAnim(FirstPersonAnimation anim, bool force = false, bool armsonly = false)
	{
		if ((m_ModelFirstPerson.active || armsonly) && (m_CurrentFirstPersonAnimation != anim || force))
		{
	...
	public virtual string GetFirstPersonModelAnimName(FirstPersonAnimation anim)
	{
		...
		{
		case FirstPersonAnimation.ThrowGrenade:
			if (!Globals.m_PlayerController.m_LastGrenadeScript.m_Mine)
			{
				return "throwGrenade";
			}
			return "throwMine";
	...
	private void PlayFirstPersonHolster(bool restart)
	{
		if (!(Globals.m_PlayerController == null) && !(Globals.m_PlayerController.m_ModelFirstPerson == null) && !(m_ModelFirstPerson == null))
		{
			...
			m_HolsterTimer = m_ModelFirstPerson.animation[GetFirstPersonWeaponAnimName(FirstPersonAnimation.Holster)].length;
		}
	}
	...
	{
		m_User = null;
		m_DroppedAmmo = UnityEngine.Random.Range(3, 10);
		m_UsesAmmo = true;
		base.transform.parent = null;
		m_ModelThirdPersonEnemy.transform.parent = base.transform;
		m_ModelThirdPersonEnemy.transform.localPosition = Vector3.zero;
		m_ModelThirdPersonEnemy.transform.localRotation = Quaternion.identity;
		m_ModelThirdPersonEnemy.transform.localScale = Vector3.one;
		m_ModelFirstPerson.SetActiveRecursively(false);
		base.transform.position = pos + new Vector3(0f, 0.1f, 0f);
		base.transform.rotation = Quaternion.Euler(0f, 0f, 90f);
		if (m_DroppedMaterial != null)
		{
			m_ModelThirdPersonEnemy.renderer.material = m_DroppedMaterial;
		}
		if ((bool)m_InteractiveVolume)
		{
			m_InteractiveVolume.EnableCollider();
		}
	}
}

...
public class InteractiveVolume : MonoBehaviour
{
	...
	{
		if (!m_VolumeReady || (!(other.gameObject.tag == "Player") && !(other.gameObject.tag == "Enemies")))
		{
			return;
		}
		m_VolumeReady = false;
		for (int i = 0; i < m_InteractiveObject.Length; i++)
		{
			...
			{
				m_InteractiveObject[i].EnableInteractiveObject(other.gameObject, other.gameObject.tag == "Player");
			}
		}
	}
	...
	private void OnTriggerExit(Collider other)
	{
		...
		for (int i = 0; i < m_InteractiveObject.Length; i++)
		{
			if ((bool)m_InteractiveObject[i])
			{
				m_InteractiveObject[i].DisableInteractiveObject(other.gameObject, other.gameObject.tag == "Player");
			}
		}
	}

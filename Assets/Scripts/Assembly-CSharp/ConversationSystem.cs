...
public class ConversationSystem : MonoBehaviour
{
	...
	private uint m_PartnerPlayingID;

	private uint m_PlayerPlayingID;
	...
	public bool IsFaceFXAnimating()
	{
		if (m_CurrentSequence != null)
		{
			if (m_CurrentSequence.m_DialogTarget == DialogSequence.DialogTarget.NPC)
			{
				...
				return SoundManager.IsEventPlaying(m_PartnerPlayingID);
			}
	...
	public void BeginConversation(Conversation_Base conversation)
	{
		...
		Globals.m_PlayerController.gameObject.GetComponent<AkListener>().enabled = false;
	...
	{
		if (sequence.m_AudioVO == string.Empty)
		{
		...
		if (sequence.m_DialogTarget == DialogSequence.DialogTarget.NPC)
		{
			...
			{
				m_PartnerPlayingID = SoundManager.TriggerEvent(sequence.m_AudioVO);
		...
		{
			m_PlayerPlayingID = SoundManager.TriggerEvent(sequence.m_AudioVO);
	...
	public void StopCurrentDialog()
	{
		if (m_CurrentSequence.m_DialogTarget == DialogSequence.DialogTarget.NPC)
		{
			...
			{
				SoundManager.StopEvent(m_PartnerPlayingID);
			}
		}
		else if (m_InteractiveObject.m_PlayerFaceFXController != null)
		{
		...
		{
			SoundManager.StopEvent(m_PlayerPlayingID);
		}
	}
	...
	public void EndConversation()
	{
		...
		Globals.m_PlayerController.gameObject.GetComponent<AkListener>().enabled = true;

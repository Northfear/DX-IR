...
public class PlayerMovement : TouchDragBase
{
	...
	protected void Update()
	{
		...
		if (Globals.m_PlayerController.m_CoverState != PlayerController.CoverState.Outside && m_IsMoving)
		{
			...
			if (!Globals.m_PlayerController.m_ForceCoverEdgeFacing || Globals.m_PlayerController.m_CoverEdge == PlayerController.CoverEdge.None)
			{
				...
				else if (num < 0f && num2 > Globals.m_PlayerController.m_CoverSwitchSidesDot && Globals.m_PlayerController.m_CoverSide != PlayerController.CoverSide.Left)
				{
					m_CoverMovingBackward = true;
				}
				if (num > 0f && num2 > 0f - Globals.m_PlayerController.m_CoverSwitchSidesDot && Globals.m_PlayerController.m_CoverSide != PlayerController.CoverSide.Right)
		...
		if (Globals.m_PlayerController.m_CoverEdge != PlayerController.CoverEdge.None && Globals.m_PlayerController.m_CoverState == PlayerController.CoverState.Inside && Globals.m_PlayerController.m_CoverCollider.gameObject.tag != "Cover_DisableCornering" && Globals.m_PlayerController.m_CoverAllowsCornering)
		{
			float num3 = 1f;
		...
		Globals.m_HUD.TurnOnVaultButton(false);
		if (Globals.m_PlayerController.m_CoverState != PlayerController.CoverState.Inside || !Globals.m_PlayerController.m_AllowUpAndOverCoverFire || !Globals.m_PlayerController.m_CoverAllowsVaulting)
		{
			return;
		}
		float num5 = 2f;
		Ray ray3 = new Ray(base.transform.position + new Vector3(0f, 3f, 0f) + base.transform.forward * num5, new Vector3(0f, -1f, 0f));
		RaycastHit hitInfo3;
		if (!Physics.Raycast(ray3, out hitInfo3, 100f, 2163457) || !(hitInfo3.point.y <= base.transform.position.y + 0.2f))
		{
			return;
		}
		Globals.m_PlayerController.m_VaultDestination = hitInfo3.point + base.transform.forward * Globals.m_PlayerController.m_CoverDistFromWall;
		if (hitInfo3.point.y < base.transform.position.y)
		{
			Globals.m_PlayerController.m_VaultDestination.y = base.transform.position.y;
		}
		ray3 = new Ray(Globals.m_PlayerController.m_VaultDestination + new Vector3(0f, 0.1f, 0f), -base.transform.forward);
		if (!Physics.Raycast(ray3, out hitInfo3, num5, 2163457))
		{
			return;
		}
		ray3 = new Ray(Globals.m_PlayerController.m_VaultDestination + new Vector3(0f, 1f, 0f), base.transform.right);
		if (!Physics.Raycast(ray3, out hitInfo3, m_PlayerCharacterController.radius + 0.1f, 2163457))
		{
			ray3 = new Ray(Globals.m_PlayerController.m_VaultDestination + new Vector3(0f, 1f, 0f), -base.transform.right);
			if (!Physics.Raycast(ray3, out hitInfo3, m_PlayerCharacterController.radius + 0.1f, 2163457))
			{
				Globals.m_HUD.TurnOnVaultButton(true);
			}
		}
	}
	...
	private void UpdateTap()
	{
		...
		if (m_CoverMove && Globals.m_PlayerController.m_CoverState == PlayerController.CoverState.Outside)
		{
			...
			{
				Globals.m_PlayerController.EnterCover(m_TargetLocation, m_TargetNormal, m_TargetCollider, true, false);
			}
	...
	private void UpdateKeyboardAlt()
	{
		if (KeyboardInput.GetKeyDown(KeyboardInput.KeyName.WalkToggle))
		{
			m_IsWalking = !m_IsWalking;
		...
		if (Globals.m_PlayerController.m_DisableController || Time.timeScale == 0f || Globals.m_PlayerController.IsDead())
		{
			return;
		}
		...
		{
			Globals.m_PlayerController.m_WeaponScript.m_WeaponState = WeaponBase.WeaponState.Idle;
			Globals.m_PlayerController.m_WeaponScript.ResetFOV(false);
			Globals.m_PlayerController.SetCoverState(PlayerController.CoverState.TransitioningFromFire);
		}
		...
		if (clampedMovementVertical <= 0f - m_DeadZoneVertical)
		{
			Globals.m_PlayerController.ExitCover(false);
		}
	}

	private void TapWorld(Vector2 screenposition)
	{
		if (Globals.m_PlayerController.m_CoverState != PlayerController.CoverState.Outside && Globals.m_PlayerController.m_CoverState != PlayerController.CoverState.Inside && Globals.m_PlayerController.m_CoverState != PlayerController.CoverState.SwitchingSides && Globals.m_PlayerController.m_CoverState != PlayerController.CoverState.Reloading)
		{
			return;
		}
		...
		if (hitInfo.collider.gameObject.layer == 8 && hitInfo.normal.y == 0f)
		{
			if (Globals.m_PlayerController.m_CoverState != PlayerController.CoverState.Outside)
			{
				...
				Vector3 vector = Vector3.Scale(hitInfo.point, hitInfo.normal);
				Vector3 vector2 = Vector3.Scale(Globals.m_PlayerController.m_CoverPoint, hitInfo.normal);
				if (Mathf.Abs(vector.magnitude - vector2.magnitude) > 0.01f)
				{
					return;
				}
			}
			vector3 = hitInfo.point + hitInfo.normal * Globals.m_PlayerController.m_CoverDistFromWall;
			vector3.y = Globals.m_PlayerController.GetGroundHeight(vector3) + 0.05f;
			m_CoverMove = true;
		...
		else
		{
			...
			{
				Vector3 vector4 = hitInfo.collider.gameObject.transform.position - base.transform.position;
				vector4.y = 0f;
				vector4.Normalize();
				hitInfo.point -= vector4 * 0.5f;
			}
			if (hitInfo.normal.y < 0.5f)
			{
				hitInfo.point += hitInfo.normal * 0.3f;
			}
			Vector3 point = hitInfo.point;
			point.y = base.transform.position.y;
			if (Vector3.SqrMagnitude(point - base.transform.position) < 1f)
			{
				return;
			}
			vector3 = hitInfo.point;
			vector3.y = Globals.m_PlayerController.GetGroundHeight(vector3 + hitInfo.normal * 0.05f) + 0.1f;
			m_CoverMove = false;
		...
		else
		{
			...
			if (m_NavAgent.CalculatePath(vector3, m_NavMeshPath))
			{
				...
				{
					vector5 = m_NavMeshPath.corners[i];
					vector5.y = 0f;
					vector6 = m_NavMeshPath.corners[i + 1];
					vector6.y = 0f;
					m_PathLength += (vector6 - vector5).magnitude;
				}
				m_CameraSpline.Reset();
				m_CameraSpline.AddPoint(base.transform.forward, base.transform.rotation, 0f, new Vector2(0f, 1f));
				vector5 = m_NavMeshPath.corners[0];
				vector5.y = 0f;
				vector6 = m_NavMeshPath.corners[1];
				vector6.y = 0f;
				Vector3 vector7 = vector6 - vector5;
				float num = vector7.magnitude;
				m_DistToNextCorner = vector7.magnitude;
				for (int j = 1; j < m_NavMeshPath.corners.Length; j++)
				{
					Vector3 vector8 = vector7;
					if (j < m_NavMeshPath.corners.Length - 1)
					{
						vector5 = m_NavMeshPath.corners[j];
						vector5.y = 0f;
						vector6 = m_NavMeshPath.corners[j + 1];
						vector6.y = 0f;
					...
					vector8 /= magnitude;
					vector7 = vector8;
					Quaternion quat = Quaternion.LookRotation(vector8);
					float timeInSeconds = num / m_PathLength;
					m_CameraSpline.AddPoint(vector8, quat, timeInSeconds, new Vector2(0f, 1f));
					num += magnitude;
				}
			}
			else
			{
				CancelTapToMove();
				Debug.Log("No path in a tap to move attempt, movetopos: " + vector3);
			}
		}
	...
	{
		if ((Globals.m_PlayerController.m_CoverState != PlayerController.CoverState.Outside && Globals.m_PlayerController.m_CoverState != PlayerController.CoverState.Inside && Globals.m_PlayerController.m_CoverState != PlayerController.CoverState.Crouch && Globals.m_PlayerController.m_CoverState != PlayerController.CoverState.Reloading) || !m_MovementPressed)
		{
			return;
		}
		float num = 0f;

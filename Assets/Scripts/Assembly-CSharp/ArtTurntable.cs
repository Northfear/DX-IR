...
public class ArtTurntable : MonoBehaviour
{
	...
	private void CheckAudio()
	{
		if (m_TurnTableModelIndex >= 9 && m_TurnTableModelIndex <= 11)
		{
			if (!m_TyrantMusicOn)
			{
				SoundManager.SetSoundSwitch("Music_Turntable", "Tyrant", Globals.m_This.gameObject);
				m_TyrantMusicOn = true;
			}
		}
		else if (m_TyrantMusicOn)
		{
			SoundManager.SetSoundSwitch("Music_Turntable", "Turntable", Globals.m_This.gameObject);
			m_TyrantMusicOn = false;
		}
		if (m_TurnTableModelIndex == 29)
		{
			SoundManager.SetSoundSwitch("Music_Turntable", "Original", Globals.m_This.gameObject);
			m_LogoMusicOn = true;
		}
		else if (m_LogoMusicOn)
		{
			SoundManager.SetSoundSwitch("Music_Turntable", "Turntable", Globals.m_This.gameObject);
			m_LogoMusicOn = false;
		}

...
public class UIButton : AutoSpriteControlBase
{
	...
	public string methodToInvokeOnDrag = string.Empty;
	...
	public override void OnInput(ref POINTER_INFO ptr)
	{
		...
		if ((ptr.evt == POINTER_INFO.INPUT_EVENT.TAP || ptr.evt == POINTER_INFO.INPUT_EVENT.RELEASE || ptr.evt == POINTER_INFO.INPUT_EVENT.RELEASE_OFF) && scriptWithMethodToInvoke != null && methodToInvokeOnRelease != string.Empty)
		{
		...
		if (ptr.evt == POINTER_INFO.INPUT_EVENT.DRAG && scriptWithMethodToInvoke != null && methodToInvokeOnDrag != string.Empty)
		{
			MethodInfo method3 = scriptWithMethodToInvoke.GetType().GetMethod(methodToInvokeOnDrag);
			method3.Invoke(scriptWithMethodToInvoke, new object[1] { ptr });
		}
		if (repeat)
		{
		...
		if (ptr.evt == whenToInvoke)
		{
			...
			if (eventsOnClick != null)
			{
				for (int i = 0; i < eventsOnClick.Length; i++)
				{
					...
					{
						SoundManager.TriggerEvent(eventsOnClick[i].SoundEventName, base.gameObject);
					}
				}
			}
		}

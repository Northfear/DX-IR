...
public class FaceFXControllerScript : MonoBehaviour
{
	...
	{
		SoundManager.StopEvent(audio_playingID);
		play_state = 0;
		switch_anim = true;
	}
	...
	{
		if (SoundManager.IsEventPlaying(audio_playingID))
		{
			Debug.Log("Audio is already playing!");
		}
		else
		{
			audio_playingID = SoundManager.TriggerEvent(audio_event, base.gameObject);
		}
	}

	public IEnumerator PlayAnimCoroutine(string animName, string animAudio)
	{
		switch_anim = false;
		play_state = 1;
		anim_eval_time = 0f;
	...
	public void PlayAnim(string animName, string animAudio)
	{
	...
	public void Update()
	{
		...
		if (animationState != null)
		{
			...
			if (play_state == 1)
			{
				if (animationState.time >= audio_start_time)
				{
					PlayAudioFunction();
					play_state = 2;
				}
			...
			else if (play_state == 2)
			{
				if (SoundManager.IsEventPlaying(audio_playingID))
				{
					anim_eval_time = SoundManager.GetEventCurrentPosition(audio_playingID) + audio_start_time;
				}
				else
				{
					play_state = 3;
			...
			else if (play_state == 3 && anim_eval_time >= animationState.length)
			{
				switch_anim = true;
				play_state = 0;
			}
			...
			}
			{
				foreach (Transform item in facefx_controller.transform)
				{
					...
					{
						Debug.Log("Warning!  The FaceFX bone pose animation could not be found: " + item.name);
						play_state = 0;
					}
				}
				return;

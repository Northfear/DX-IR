...
public class UIRadioBtn : AutoSpriteControlBase, IRadioButton
{
		...
		if (ptr.evt == whenToInvoke)
		{
			...
			if (eventsToPlay != null)
			{
				for (int i = 0; i < eventsToPlay.Length; i++)
				{
					...
					{
						SoundManager.TriggerEvent(eventsToPlay[i].SoundEventName, base.gameObject);
					}
				}
			}
	...
	protected void ManuallyPopOtherButtonsInGroup()
	{
		if (group == null)
		{
			return;
		...
		for (int i = 0; i < group.buttons.Count; i++)
		{
			if ((UIRadioBtn)group.buttons[i] != this)
			{
				((UIRadioBtn)group.buttons[i]).ManuallySetGroupValue(false);
			}
		}
	}
	...
	protected virtual void ManuallySetGroupValue(bool val)
	{
		btnValue = val;
		SetButtonState(false);
	}

	public void ManuallySetValue(bool val)
	{
		btnValue = val;
		if (btnValue)
		{
			ManuallyPopOtherButtonsInGroup();
		}
		SetButtonState(false);
	}

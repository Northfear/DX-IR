...
public class UIScrollList : MonoBehaviour, IEZDragDrop, IUIObject
{
	...
	public bool m_UseMaxSnapDuration;

	public float m_MaxSnapDuration = 2f;
	...
	protected void ScrollListTo_Internal(float pos)
	{
		if (!float.IsNaN(pos) && !(mover == null))
		{
			...
			{
				float num = ((direction != DIRECTION.TtoB_LtoR) ? (-1f) : 1f);
				mover.transform.localPosition = Vector3.up * num * Mathf.Clamp(amtOfPlay, 0f, amtOfPlay) * pos;
			}
			else
			{
				float num2 = ((direction != DIRECTION.TtoB_LtoR) ? 1f : (-1f));
	...
	{
		if (m_UseMaxSnapDuration)
		{
			scrollTime = Mathf.Min(scrollTime, m_MaxSnapDuration);
		}
		snappedItem = item;
		if (newItems.Count != 0)
		{
		...
		for (int i = 0; i < newItems.Count; i++)
		{
			...
			if (orientation == ORIENTATION.HORIZONTAL)
			{
				if (index > 0)
				{
					...
					x = ((direction != DIRECTION.TtoB_LtoR) ? (iUIListObject.transform.localPosition.x - iUIListObject.BottomRightEdge.x - itemSpacingActual + iUIListObject2.TopLeftEdge.x) : (iUIListObject.transform.localPosition.x + iUIListObject.BottomRightEdge.x + itemSpacingActual - iUIListObject2.TopLeftEdge.x));
				}
				else
				{
					if (spacingAtEnds)
					...
					x = ((direction != DIRECTION.TtoB_LtoR) ? (viewableAreaActual.x * 0.5f - iUIListObject2.BottomRightEdge.x - ((!spacingAtEnds) ? 0f : itemSpacingActual) - extraEndSpacingActual) : (viewableAreaActual.x * -0.5f - iUIListObject2.TopLeftEdge.x + ((!spacingAtEnds) ? 0f : itemSpacingActual) + extraEndSpacingActual));
				}
				switch (alignment)
				{
			...
			else
			{
				if (index > 0)
				{
					flag = true;
					iUIListObject = items[index - 1];
					y = ((direction != DIRECTION.TtoB_LtoR) ? (iUIListObject.transform.localPosition.y - iUIListObject.BottomRightEdge.y + itemSpacingActual + iUIListObject2.TopLeftEdge.y) : (iUIListObject.transform.localPosition.y + iUIListObject.BottomRightEdge.y - itemSpacingActual - iUIListObject2.TopLeftEdge.y));
				}
				else
				{
					if (spacingAtEnds)
					...
					y = ((direction != DIRECTION.TtoB_LtoR) ? (viewableAreaActual.y * -0.5f - iUIListObject2.BottomRightEdge.y + ((!spacingAtEnds) ? 0f : itemSpacingActual) + extraEndSpacingActual) : (viewableAreaActual.y * 0.5f - iUIListObject2.TopLeftEdge.y - ((!spacingAtEnds) ? 0f : itemSpacingActual) - extraEndSpacingActual));
				}
				switch (alignment)
				{
				case ALIGNMENT.CENTER:
	...
	protected void CalcSnapItem()
	{
		...
		else
		{
			...
			if (orientation == ORIENTATION.HORIZONTAL)
			{
				float num6 = ((direction != DIRECTION.TtoB_LtoR) ? 1f : (-1f));
			...
			float num8 = ((direction != DIRECTION.TtoB_LtoR) ? (-1f) : 1f);
			iUIListObject = items[num5];
